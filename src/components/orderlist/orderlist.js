...
const OrderList = (keys) => {
  ...
  const thisMonth = myDate.getUTCMonth();
  const thisDay = myDate.getUTCDate();
  ...
  dateWrapper.append('Please provide your visit date', dateInput);
  dateInput.setAttribute('type', 'date');
  dateInput.setAttribute('min', `${thisYear}-0${thisMonth + 1}-${thisDay}`);
  dateInput.setAttribute('value', `${thisYear}-0${thisMonth + 1}-${thisDay}`);
  dateInput.setAttribute('max', `${thisYear + 1}-0${thisMonth + 1}-${thisDay}`);
